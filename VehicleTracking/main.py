...
ocrModel = PaddleOCR(use_angle_cls=True, use_space_char=True, lang='en', use_gpu= True if device == 'cuda' else False)
deepsort = DeepSort(
                max_age=20,
                n_init=15,
                nms_max_overlap=0.5,
...
def main():
    ...
    frame = cv2.resize(frame,(1080,600),interpolation=cv2.INTER_AREA)
    ...
    if(len(trackClassList) != 0):
        for track in tracker:
            ...
            # draw boundingBox and putText object-name
...
#               personCount, vehicleCount, detect licensePlate
...
checkboxList = [personCountCheckBox, vehicleCountCheckBox, licensePlateCheckBox]

closeBtn = Button(text="Close",command=close,width=6,height=1)
personBtn = Button(text="Person",command=lambda: modifyTrackingObject(0,trackClassList,checkboxList),width=10,height=1)
vehicleBtn = Button(text="vhicle",command=lambda: modifyTrackingObject([1,2,3,5,7],trackClassList,checkboxList),width=10,height=1)
